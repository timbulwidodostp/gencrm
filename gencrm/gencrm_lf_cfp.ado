...
program gencrm_lf_cfp
  ...
  *** likelihood function for probit link
  if ( "$Link" == "probit" ) {	
    ...
    * equation for first value of Y
    qui replace `lnf' = ln(normal(`tau1' - `xb_c' - `xb_f1' - `xb_p')) if $ML_y == `y_1'
		
    * build equations for middle values of Y
    if ( $nCat == 3 ) {
	  qui replace `lnf' = ln(1 - normal(`tau1' - `xb_c' - `xb_f1' - `xb_p')) +  ///
		                  ln(    normal(`tau2' - `xb_c' - `xb_f2' - `xb_p'*`phi2')) if $ML_y == `y_2'
	}
	
	if ( $nCat > 3 ) {
	  forval k = 2/$nCatm1 {
	    local meqn_a `" ln(1 - normal(`tau1' - `xb_c' - `xb_f1' - `xb_p')) + "'
        local meqn_c `" ln(    normal(`tau`k'' - `xb_c' - `xb_f`k'' - `xb_p'*`phi`k'')) "'
    
	    local meqn_b ""
	    local m = `k' - 1
        forval n = 2/`m' {
          local meqn_b `" `meqn_b' ln(1 - normal(`tau`n'' - `xb_c' - `xb_f`n'' - `xb_p'*`phi`n'')) + "'
        }
	...
	* build equation for last value of Y
	local eqn `" ln(1 - normal(`tau1' - `xb_c' - `xb_f1' - `xb_p')) "'
	forval o = 2/$nCatm1 {
      local eqn `" `eqn' + ln(1 - normal(`tau`o'' - `xb_c' - `xb_f`o'' - `xb_p'*`phi`o'')) "'
    }
	qui replace `lnf' = `eqn' if $ML_y == `y_`M''
  }
  ...
  *** likelihood function for cloglog link
  if ( "$Link" == "cloglog" ) {	
    ...
    * equation for first value of Y
    qui replace `lnf' = ln(1 - exp(-exp(`tau1' - `xb_c' - `xb_f1' - `xb_p'))) if $ML_y == `y_1'
		
    * build equations for middle values of Y
    if ( $nCat == 3 ) {
	  qui replace `lnf' = ln(exp(-exp(`tau1' - `xb_c' - `xb_f1' - `xb_p'))) +  ///
		                  ln(    1 - exp(-exp(`tau2' - `xb_c' - `xb_f2' - `xb_p'*`phi2'))) if $ML_y == `y_2'
	}
	
	if ( $nCat > 3 ) {
	  forval k = 2/$nCatm1 {
	    local meqn_a `" ln(exp(-exp(`tau1' - `xb_c' - `xb_f1' - `xb_p'))) + "'
        local meqn_c `" ln(    1 - exp(-exp(`tau`k'' - `xb_c' - `xb_f`k'' - `xb_p'*`phi`k''))) "'
    
	    local meqn_b ""
	    local m = `k' - 1
        forval n = 2/`m' {
          local meqn_b `" `meqn_b' ln(exp(-exp(`tau`n'' - `xb_c' - `xb_f`n'' - `xb_p'*`phi`n''))) + "'
        }
	...
	* build equation for last value of Y
	local eqn `" ln(exp(-exp(`tau1' - `xb_c' - `xb_f1' - `xb_p'))) "'
	forval o = 2/$nCatm1 {
      local eqn `" `eqn' + ln(exp(-exp(`tau`o'' - `xb_c' - `xb_f`o'' - `xb_p'*`phi`o''))) "'
    }
	qui replace `lnf' = `eqn' if $ML_y == `y_`M''
  }
end
